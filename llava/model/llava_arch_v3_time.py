        ...
        if current_total > self.budget:
            if self.U.shape[1] > 0:
                ...
                _, keep_indices = torch.topk(
                    combined_scores,
                    min(fg_budget, self.mem1_buffer.size(0)),
                )
...
class LlavaMetaForCausalLM(BaseLlavaMetaForCausalLM):
    def stream_compress_video_features(self, image_feature, verbose=True):
        print("v3_time")
        # print('kmax',int(os.getenv("FOSS_K_MAX", getattr(self.config, "foss_k_max", 256))))
        cache = FOSSCache(
        ...
        # print("Final retained token count per frame:")
        # for frame_idx, token_count in enumerate(final_counts):
        #     print(f"Frame {frame_idx}: {token_count} tokens")
